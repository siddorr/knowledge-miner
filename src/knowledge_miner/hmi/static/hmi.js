...
import { createBuildModule } from "./hmi/build.js";
...
const liveUpdates = createLiveUpdatesModule({
  state,
  AUTH_ENABLED,
  POLL_ACTIVE_MS,
  POLL_BACKGROUND_MS,
...
function schedulePoll() {
  return liveUpdates.schedulePoll();
}
...
function activeTopic() {
  return build.activeTopic();
}

function renderBuildDetails() {
  return build.renderBuildDetails();
}

function renderBuildSources() {
  return build.renderBuildSources();
}

function renderBuildTopics() {
  return build.renderBuildTopics();
}

function setBuildTab(tab) {
  return build.setBuildTab(tab);
}

function sourceFingerprint(value) {
  return build.sourceFingerprint(value);
}

function emptyTopicCoverage() {
  return build.emptyTopicCoverage();
}

function mergeTopicCoverage(queueItems, sources) {
  return build.mergeTopicCoverage(queueItems, sources);
}

function applyActiveTopicCoverageToShell() {
  return build.applyActiveTopicCoverageToShell();
}
...
function handleBuildTopicClick(event) {
  return build.handleBuildTopicClick(event);
}

function createNewTopic() {
  return build.createNewTopic();
}

function handleAddSource(event) {
  return build.handleAddSource(event);
}

function handleBulkSource(event) {
  return build.handleBulkSource(event);
}

function handleBuildQuery(event) {
  return build.handleBuildQuery(event);
}
...
async function runPollCycle() {
  return liveUpdates.runPollCycle();
}

async function runLiveRefresh(eventType = "queue_updated") {
  return liveUpdates.runLiveRefresh(eventType);
}

function queueLiveRefresh(eventType) {
  return liveUpdates.queueLiveRefresh(eventType);
}

function openLiveUpdatesChannel() {
  return liveUpdates.openLiveUpdatesChannel();
}

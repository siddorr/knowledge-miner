...
0. [ ] P0 - Fix Review page run-context gap when badge shows pending sources
- Goal: if the shell shows `N sources need review`, the Review page must always resolve and display those rows or explicitly ask the operator to choose the owning run.
- Problem observed:
  - HMI badge/count is driven by `/v1/work-queue`, but Review row loading is driven by `state.latest.discovery`.
  - In current failure mode, `run_id` is `null` on Review navigation, so the badge shows `18` while Review never issues `/v1/discovery/runs/{run_id}/sources`.
- Tasks:
  - On entering `Review`, if `state.latest.discovery` is empty and pending review work exists, resolve run context from `/v1/runs/latest` or another explicit backend mapping.
  - If pending review sources belong to multiple runs, show an explicit run selector/banner instead of empty review content.
  - Prevent task badges from advertising actionable review work when the page cannot resolve a run context.
  - Add telemetry/logging for Review autoload decisions:
  ...
  - Add API/UI tests:
    ...
    - no run context available -> operator sees actionable message, not silent empty state
